...
1.  В папке backend\src\main\resources\ создайте файл **application.properties** и скопируйте в него содержимое из файла **application.properties.example**.
	...
	**Note**: файл **application.properties** содержит реальные ключи проекта и не должен сохраняться в репозиторий.
	...
	Для того, чтобы указать токен для Gyazo, необходимо зарегистрироваться на сайте [Gyazo](https://gyazo.com/welcome) и [зарегистрировать приложение](https://gyazo.com/oauth/applications) и прочитать три строчки [документации](https://gyazo.com/api/docs), что видеть, как работать с API.

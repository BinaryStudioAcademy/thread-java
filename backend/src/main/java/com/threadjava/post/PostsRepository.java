...
import com.threadjava.post.dto.AllPostQueryResult;
...
public interface PostsRepository extends CrudRepository<Post, UUID> {
    ...
    @Query("SELECT new com.threadjava.post.dto.AllPostQueryResult(p.id, p.body, " +
    ...
    List<AllPostQueryResult> findAllPosts(@Param("userId") UUID userId, Pageable pageable);
}